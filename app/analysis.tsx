...
export default function AnalysisScreen() {
    ...
    const weeklyData = useMemo(() => {
        ...
        for (let i = 0; i < 7; i++) {
            ...
            const dateStr = formatLocalDate(date); // Use local timezone instead of UTC

            // Debug: Check date matching
            if (__DEV__ && dailyLogs && dailyLogs.length > 0 && i === 0) {
                console.log('📊 Daily logs dates:', dailyLogs.map(l => l.log_date));
